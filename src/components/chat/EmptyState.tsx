...
export const EmptyState: React.FC<EmptyStateProps> = ({ onSelectPrompt }) => {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-4 max-w-2xl mx-auto w-full">
      {/* ╭─ Welcome Banner ──────────────────────────────────────────╮ */}
      <div className="w-full border border-[#2c2e3a] rounded-lg bg-[#14151a] mb-4">
        ...
        <div className="px-4 py-3 space-y-1.5">
          <div className="text-base font-bold text-[#f3f3ee]">
            Azal Labs
          </div>
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-[#6b6e79]">
            <span>النسخة 2.1.0</span>
            <span>•</span>
            <span>بيئة متكاملة للتطوير وإدارة المهام</span>
          </div>
          <div className="pt-1 text-[13px] text-[#9da0a8] leading-relaxed">
      ...
      <div className="grid grid-cols-2 gap-2 w-full">
        {suggestions.map((item, idx) => (
          <button
            ...
            className="text-right p-2.5 border border-[#2c2e3a] rounded-lg bg-[#0d0e11] hover:bg-[#14151a] hover:border-[#cc785c]/40 transition-all cursor-pointer group"
          >
            <div className="text-[11px] font-bold text-[#cc785c] mb-1">
              {item.cmd}
            </div>
            <div className="text-[11px] text-[#6b6e79] group-hover:text-[#9da0a8] transition-colors leading-relaxed">
              {item.label}
            </div>
          </button>
        ))}
